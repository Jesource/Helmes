...
docker run --name $CONTAINER_NAME -e POSTGRES_PASSWORD=$POSTGRES_PASSWORD -d -p 5432:5432 $IMAGE_NAME


# Wait for the PostgreSQL container to start
echo "Waiting 2 seconds for PostgreSQL container to start..."
sleep 2
...
docker exec -it $CONTAINER_NAME psql -U $POSTGRES_USER -d $DATABASE_NAME -c "CREATE USER $OASYS_USER WITH PASSWORD '$OASYS_USER_PASSWORD';"
docker exec -it $CONTAINER_NAME psql -U $POSTGRES_USER -d $DATABASE_NAME -c "GRANT ALL PRIVILEGES ON DATABASE $DATABASE_NAME TO $OASYS_USER;"
# Let the application to create and manage tables
docker exec -it $CONTAINER_NAME psql -U $POSTGRES_USER -d $DATABASE_NAME -c "GRANT ALL PRIVILEGES ON SCHEMA public TO $OASYS_USER; COMMIT;"
